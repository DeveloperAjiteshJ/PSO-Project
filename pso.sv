...
module pso_rc_tuner;
  parameter int  N_PARTICLES       = 12;
  parameter int  N_ITER            = 25;
  ...
  parameter int  MAX_WORSE_STEPS   = 6;
  parameter int  ENABLE_GREEDY_REPAIR = 1;

  localparam real TWO_PI = 6.283185307179586;

  // Particle state (64-bit real encoded as logic)
  logic [63:0] pos_R     [0:N_PARTICLES-1];
  logic [63:0] pos_C     [0:N_PARTICLES-1];
  logic [63:0] vel_R     [0:N_PARTICLES-1];
  logic [63:0] vel_C     [0:N_PARTICLES-1];
  logic [63:0] pbest_R   [0:N_PARTICLES-1];
  logic [63:0] pbest_C   [0:N_PARTICLES-1];
  logic [63:0] pbest_err [0:N_PARTICLES-1];
  int          off_path_cnt [0:N_PARTICLES-1];
  int          active       [0:N_PARTICLES-1];

  // Global best
  logic [63:0] gbest_R_bits, gbest_C_bits, gbest_err_bits;
  int          gbest_idx;

  // Loop indices and temps
  int i, j, it;
  int active_count, last_active_idx;

  // Temps for math
  logic [63:0] r1_b, r2_b;
  logic [63:0] new_vr_b, new_vc_b;
  logic [63:0] new_pr_b, new_pc_b;
  logic [63:0] new_vr_adj_b, new_vc_adj_b;
  logic [63:0] fc_b, err_b;
  real         prev_best_err;

  // Greedy repair temps
  logic [63:0] fc_tmp_b, err_orig_b, err_rep_b;
  logic [63:0] rep_R_b, rep_C_b;

  // Iteration logging helpers
  int  disabled_this_iter [0:N_PARTICLES-1];
  int  cnt_before, act_before;
  int  stop_now;
  int  first;
  ...
  function automatic logic [63:0] real_to_bits(input real x);
    return $realtobits(x);
  endfunction
  ...
  // RNG: uniform [0,1)
  // -----------------------------------------------------------------------
  task automatic rng01_bits(output logic [63:0] ubo);
    real ur;
  ...
  // -----------------------------------------------------------------------
  task automatic velocity_update_bits(
    input  logic [63:0] pos_R_b, pos_C_b, vel_R_b, vel_C_b,
                        pbest_R_b, pbest_C_b, gbest_R_b, gbest_C_b,
                        r1_bi, r2_bi,
    output logic [63:0] vel_R_out_b, vel_C_out_b
  );
  ...
  // -----------------------------------------------------------------------
  task automatic fitness_bits(
    input  logic [63:0] R_b, C_b,
    output logic [63:0] fc_bo, err_bo
  );
    real R, C, fc, de, perr;
  ...
  // -----------------------------------------------------------------------
  task automatic update_gbest();
    int  jj, found;
    real best_e, e;
    best_e = 1.0e300;
    found  = 0;
    for (jj = 0; jj < N_PARTICLES; jj++) begin
      if (active[jj]) begin
  ...
  // -----------------------------------------------------------------------
  task automatic dpsh_update(
    input  logic [63:0] curr_err_b, pbest_err_prev_b,
    ref    int          cnt, act
  );
    real ce, pe;
  ...
  // -----------------------------------------------------------------------
  task automatic position_update_bits(
    input  logic [63:0] pos_R_b, pos_C_b, vel_R_b, vel_C_b,
    output logic [63:0] pos_R_out_b, pos_C_out_b, vel_R_out_b, vel_C_out_b
  );
    ...
    for (reflect_iter = 0; reflect_iter < 4; reflect_iter++) begin
      if (nr < R_MIN) begin
      ...
      if ((nr >= R_MIN && nr <= R_MAX) && (nc >= C_MIN && nc <= C_MAX))
        break;
    end

    if (nr < R_MIN) nr = R_MIN; else if (nr > R_MAX) nr = R_MAX;
    if (nc < C_MIN) nc = C_MIN; else if (nc > C_MAX) nc = C_MAX;
  ...
  // -----------------------------------------------------------------------
  task automatic repair_choose_best(
    input  logic [63:0] Rin_b, Cin_b,
    output logic [63:0] Rout_b, Cout_b
  );
    real         R_in, C_in, R1, C1loc, R2, C2loc, Cstar, Rstar;
    logic [63:0] fc_b1, err_b1, fc_b2, err_b2;
    real         e1, e2;
    ...
    if (Cstar < C_MIN)      C1loc = C_MIN;
    else if (Cstar > C_MAX) C1loc = C_MAX;
    else                    C1loc = Cstar;
    R1 = R_in;

    // Candidate 2: fix C, solve R*
    Rstar = 1.0 / (TWO_PI * C_in * F_DESIRED);
    if (Rstar < R_MIN)      R2 = R_MIN;
    else if (Rstar > R_MAX) R2 = R_MAX;
    else                    R2 = Rstar;
    C2loc = C_in;
  ...
  initial begin
    ...
    $display("Inputs: F_DESIRED=%.6f Hz, R_MIN=%.3f, R_MAX=%.3f, C_MIN=%.3e, C_MAX=%.3e",
             F_DESIRED, R_MIN, R_MAX, C_MIN, C_MAX);
    $display("Particles: %0d, Iterations: %0d", N_PARTICLES, N_ITER);
    $display("-----");

    // Initialize particles and print initial states
    for (i = 0; i < N_PARTICLES; i++) begin
    ...
    // Iterations
    for (it = 0; it < N_ITER; it++) begin
      // Clear disabled list for this iteration
      for (j = 0; j < N_PARTICLES; j++) disabled_this_iter[j] = 0;
      ...
      $write("Iter %0d: GBEST=P%0d R=%.3f C=%.3e fc=%.6f err=%.6f%% | Disabled:",
             it, gbest_idx, gR, gC, gfc, gerr);
      first = 1;
      for (j = 0; j < N_PARTICLES; j++) begin
    ...
    $display("Final: GBEST=P%0d R=%.3f C=%.3e fc=%.6f Err=%.6e%%",
             gbest_idx, Rf, Cf, fcf, bits_to_real(gbest_err_bits));
    $display("---------------");
    $finish;
  end
endmodule
